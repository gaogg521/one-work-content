...
TDD metodolojisini takip ederek güvenilir, bakım yapılabilir testler yazmak için kapsamlı Rust 测试 desenleri.

## Ne Zaman Kullanılır

- Yeni Rust fonksiyonları, metotları veya trait'leri yazma
- Mevcut koda 测试 kapsamı ekleme
...
1. **Hedef kodu tanımla** — 测试 edilecek fonksiyon, trait veya modülü bul
2. **Bir 测试 yaz** — `#[cfg(test)]` modülünde `#[t`#[t`st]`lan, parametreli testler için rstest veya property-based testler için proptest
3. **Bağımlılıkları mock'la** — 测试 altındaki birimi izole etmek için mockall kullan
...
### Modül Seviyesi 测试 Organizasyonu
...
- Implementasyon değil, davranışı 测试 edin
- Senaryoyu açıklayan açıklayıcı 测试 isimleri kullanın
- Daha iyi hata mesajları için `assert!` yerin`assert_eq!``` tercih edin
- Daha temiz hata çıktısı için `Result` döndüren testle`?`?`?` kullanın
- Testleri bağımsız tutun — paylaşılan mutable state yok

**YAPMAYIN:**
- `Result::is_err()` 测试 edebiliyorsanız `#[shoul`#[shoul`_panic]`ayın
...
- Testlerde `sleep()` kullanmayın — channel'lar, barrier'lar vey`tokio::time::pause()``` kullanın
