...
description: Idiomatic C# and .NET patterns, conventions, 依赖 injection, async/await, and best practices for building robust, maintainable .NET applications.
origin: ECC
---
...
- Designing 服务 architectures with ASP.NET Core

## Core Principles

### 1. Prefer Immutability

Use records and init-only properties for data models. Mutability 应该 be an explicit, justified choice.
...
Be 清空 about nullability, access modifiers, and intent.
...
Use interfaces for 服务 boundaries. Register via DI container.
...
Bind 配置 sections 迁移到 strongly-typed objects.
...
返回 explicit 成功/failure instead of throwing for expected failures.
...
| Anti-Pattern | Fix |
|---|---|
| `async void` methods | 返回 `T__CO`sk`__sk`except 事件 handlers) |
| `.Result` `.Wait()``_CODE_2____COD`t`__t`it`t` |
| `catch (Exception) { }` | 处理 or rethrow with context |
...
| Mutable `static` state | Use DI scopi`ConcurrentDictionary`y`y`y` |`y`
| `string.Format` in loops | Use `Stri`Stri``gBuilder`terpolated 字符串 handlers |