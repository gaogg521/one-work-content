---
name: sql-queries
description: 在所有主流数据仓库方言（Snowflake、BigQuery、Databricks、PostgreSQL等）中编写正确、高性能的SQL。用于编写查询、优化慢SQL、方言间转换或使用CTE、窗口函数或聚合构建复杂分析查询时。
user-invocable: False
---

# SQL查询技能

在所有主流数据仓库方言中编写正确、高性能、可读的SQL。
...
-- 解析JSON
column:key::string  -- VARIANT的点表示法
...
- 对分区内的频繁过滤列使用聚簇
- 对大规模基数估计使用`APPROX_COUNT_DISTINCT()`
- 避免`SELECT *` -- 按扫描字节数计费
- 使用`DECLARE`和`SET`进行参数化脚本
- 执行大查询前使用dry run预览查询成本
...
**性能提示：**
- 使用Delta Lake的`OPTIMIZE`和`ZORDER`提高查询性能
- 利用Photon引擎进行计算密集型查询
...
1. **语法错误**：检查方言特定语法（例如，BigQuery中没有`ILIKE`，`SAFE_DIVIDE`仅在BigQuery中）
2. **列未找到**：根据模式验证列名 -- 检查拼写错误、大小写敏感性（PostgreSQL对带引号的标识符区分大小写）
3. **类型不匹配**：比较不同类型时显式转换（`CAST(col AS DATE)`，`col::DATE`）
4. **除以零**：使用`NULLIF(denominator, 0)`或方言特定的安全除法
5. **列不明确**：在JOIN中始终用表别名限定列名
6. **Group by错误**：所有非聚合列必须在GROUP BY中（BigQuery除外，它允许按别名分组）
