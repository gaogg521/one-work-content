...
description: 生成 a 完成 模型 Context 协议 服务器 项目 in Java using the official MCP Java SDK with reactive streams and optional Spring Boot integration.
---

# Java MCP 服务器 生成器

生成 a 完成, production-ready MCP 服务器 in Java using the official Java SDK with Maven or Gradle.

## 项目 Generation

When asked 迁移到 创建 a Java MCP 服务器, 生成 a 完成 项目 with this structure:
...
1. **Ask for 项目 name and 包**
2. **Choose 构建 tool** (Maven or Gradle)
3. **生成 all 文件** with proper 包 structure
4. **Use Reactive Streams** for 异步 handlers
5. **Include comprehensive logging** with SLF4J
6. **添加 tests** for all handlers
7. **Follow Java conventions** (camelCase, PascalCase)
8. **Include 错误 handling** with proper responses
9. **Document public APIs** with Javadoc
10. **Provide both 同步 and 异步** 示例