...
description: Plan and 构建 production-ready Rust CLI tools using clap for argument parsing, with subcommands, config file 支持, colored 输出, and proper 错误 handling. Uses interview-driven planning 迁移到 clarify 命令, 输入/输出 formats, and distribution strategy before writing any code.
tags:
- rust
- cli
- clap
...
Use this skill when you 需要 迁移到:

- Scaffold a new Rust CLI tool from scratch with clap
- 添加 subcommands 迁移到 an existing CLI application
- 实现 config file loading (TOML/JSON/YAML)
- Set up proper 错误 handling with anyhow/thiserror
- 添加 colored and formatted terminal 输出
- Structure a CLI project for distribution via cargo 安装 or GitHub releases

## Phase 1: Explore (Plan Mode)

Enter plan mode. Before writing any code, 探索 existing project:

### If extending an existing project
- 查找 `Cargo.toml` and 检查 current dependencies (clap 版本, serde, tokio, etc.)
- Locate the CLI entry point (`src/main.rs` or `sr`sr`/cli.rs`
- 检查 if clap is using derive macros or builder pattern
- Identify existing subcommand structure
- Look for existing 错误 types, config structs, and 输出 formatting
- 检查 if there's a `src/lib.rs` separating library logic from CLI

### If starting from scratch
- 检查 the workspace for any existing Rust projects or workspace `Cargo.toml`
- Look for a `.cargo/config.toml` with custom settings
- 检查 for `rust-toolchain.toml` 迁移到 know the target Rust edition

## Phase 2: Interview (AskUserQuestion)

Use AskUserQuestion 迁移到 clarify 环境要求. Ask in rounds.

### Round 1: Tool purpose and 命令
...
### Round 2: Subcommands (if multi-命令)
...
写入 a concrete implementation plan covering:

1. **Project structure** — `Cargo.toml` dependencies, `s`s`c/`ile layout
2. **CLI definition** — clap derive structs for all 命令, args, and flags
3. **Config loading** — config file format and 合并 strategy with CLI args
4. **Core logic** — main functions for each subcommand, separated from CLI layer
5. **错误 types** — 错误 enum or anyhow 用法, user-facing 错误 messages
6. **输出 formatting** — colored 输出, JSON mode, progress indicators
...
After approval, 实现 following this order:

### Step 1: Project 设置 (Cargo.toml)
...
### Step 5: Colored 输出 and formatting
...
Keep clap structs and argument parsing in `cli.rs`. Put business logic`commands/`/`/`. This makes the core logic testable without invoking the CLI.

### Use stderr for status, stdout for data
Human-readable messages (progress, 成功, errors) go 迁移到 `stderr`. Machine-readable data goes`stdout`_CODE_2__ean`toolname status --format json | jq '.items'`ms'`.`ms'`

### Respect NO_COLOR
检查 the `NO_COLOR` environment variable and 禁用 colors when set:
```rust
...
### Exit codes
Use meaningful exit codes: 0 for 成功, 1 for general errors, 2 for 用法 errors (clap handles this automatically).
...
- [ ] Config file has sensible defaults and doesn't 错误 when missing
- [ ] `--format json` outputs valid, parseable JSON 迁移到 stdout
- [ ] Errors 显示 context (file paths, what went wrong, how 迁移到 fix it)
- [ ] Integration tests 验证 CLI behavior end-迁移到-end
- [ ] `cargo clippy` passes with no warnings
- [ ] `cargo fmt` has been 运行