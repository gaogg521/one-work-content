...
description: Azure Cosmos DB Python SDK（NoSQL API）。用于文档的增删改查、查询、容器管理以及全球分布式数据操作。 触发词：\"cosmos db\", \"CosmosClient\", \"container\", \"document\", \"NoSQL\", \"partition key\"。
...
try:
    ...
    if e.status_code == 404:
        ...
        print(f"速率受限。重试等待: {e.headers.get('x-ms-retry-after-ms')}ms")
    else:
        raise
```
...
1. **始终指定分区键** 用于点读取和查询
2. **使用参数化查询** 以防止注入并提升缓存性能
3. **避免跨分区查询** 如果可能的话
4. **使用 `upsert_item`** 实现幂等写入
5. **使用异步客户端** 用于高吞吐量场景
6. **设计分区键** 以实现均匀的数据分布
7. **使用 `read_item`** 替代查询来检索单个文档

## 参考文件

| 文件 | 内容 |
|------|----------|
| [references/partitioning.md](references/partitioning.md) | 分区键策略、层次键、热点分区检测与缓解 |
| [references/query-patterns.md](references/query-patterns.md) | 查询优化、聚合、分页、事务、变更流 |
| [scripts/setup_cosmos_container.py](scripts/setup_cosmos_container.py) | 用于创建带分区、吞吐量和索引的容器的 CLI 工具 |
