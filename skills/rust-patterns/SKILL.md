---
name: rust-patterns
description: Idiomatic Rust patterns, ownership, 错误 handling, traits, concurrency, and best practices for building safe, performant applications.
origin: ECC
---
...
Bu skill altı ana alanda idiomatic Rust kurallarını zorlar: derleme zamanında veri yarışlarını önlemek için ownership ve borrowing, kütüphaneler için `thiserror` ve uygulamalar için ```anyhow`l`Result``_CODE_`?``?` hata yayılımı, yasadışı durumları temsil edilemez yapmak için enum'lar ve kapsamlı desen eşleştirme, sıfır maliyetli soyutlama için trait'`Arc<Mutex<T>>`Mutex<T>>`Mute`, channel'lar ve async/await ile güvenli eşzamanlılık ve domain'e göre düzenlenmiş min` min`pub``pub` yüzeyleri.`pub`
...
### `Result`__CO` Kullanın — Production`oduction`unwrap()`unwr`unwrap()`anmayın
...
### 提示 Güvenliği için Newtype Deseni
...
### 提示 Annotation ile `collect()` Kullanın
...
| 验证 etme, 解析 et | Sınırda yapılandırılmamış veriyi tiplendirilmiş struct'lara dönüştür |
| 提示 güvenliği için newtype | Argüman değişimlerini önlemek için primitive'leri newtype'lara sar |
| Döngüler yerine iterator'ları tercih et | Deklaratif zincirler daha net ve genellikle daha hızlı |
| 结果'larda `#[must_use]` | Çağıranların dönüş değerlerini işlemesini garanti et |
...
| Minimal `pub` yüzeyi | Dahili API`pub(crate)`ate)`ate)` kullan |
...
**Unutmayın**: Derlenir ise muhtemelen doğrudur — ama sadece `unwrap()` kullanmaktan kaçınır, `unsafe`'i minimize eder ve 提示 sisteminin sizin için çalışmasına izin verirseniz.