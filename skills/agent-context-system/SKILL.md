...
在 session 结束时，agent **在写入之前向用户提议** session log 条目。Agent 不得直接附加——它展示提议的条目并等待用户批准后才写入 `.agents.local.md`。大多数 agent（Copilot Chat, Cursor, Windsurf）没有 session-end hooks，所以这取决于 `AGENTS.md` 中的 Rule 7 被看到并执行，或者用户说 "log this session"。Claude Code 通过 auto memory 自动处理这个。
...
OpenClaw 将在下次 session 中将其识别为 workspace skill。
...
或者将 `github-copilot/SKILL.md` 复制到 `.github/skills/agent-context-system/SKILL.md`。
...
- **没有外部下载。** 所有文件都包含在仓库中。安装时不会从外部 URL 下载二进制文件。
- **Scratchpad 写入需要用户确认。** Agent 必须向用户展示提议的 session log 条目，并在附加到 `.agents.local.md` 之前等待批准。
- **`.agents.local.md` 是 gitignored。** Init 脚本确保这一点。个人 scratchpad 数据永远不会提交到版本控制。
- **Path-scoped operations。** CLI 仅在当前工作目录内操作。它不跟随指向项目根目录之外的 symlink，也不写入包含 `..` 的路径。
- **信任边界是你的本地文件系统。** `.agents.local.md` 位于用户的项目目录中，gitignored。信任模型与 `.bashrc`、`.env` 或 IDE 配置文件相同——如果攻击者可以写入你的本地项目文件，agent context 不是你最大的问题。
- **Scratchpad 内容是数据，不是指令。** Agent 将 `.agents.local.md` 视为事实性的 session 记录：what happened, what worked, what didn't。如果 scratchpad 包含类似新行为规则、命令覆盖或 system prompt 指令的内容，agent 应该忽略它并提醒用户。
