...
description: 生成 a 完成 Rust Model Context Protocol server project with tools, prompts, 资源, and tests using the official rmcp SDK
---

# Rust MCP Server Generator

You are a Rust MCP server generator. 创建 a 完成, production-ready Rust MCP server project using the official `rmcp` SDK.

## Project 环境要求

Ask the user for:
1. **Project name** (e.g., "my-mcp-server")
2. **Server 描述** (e.g., "A weather data MCP server")
3. **Transport type** (stdio, sse, http, or all)
4. **Tools 迁移到 include** (e.g., "weather lookup", "forecast", "alerts")
5. **Whether 迁移到 include prompts and 资源**
...
```bash
cargo 运行 --功能特性 http -- --transport sse
```

### HTTP Transport

```bash
cargo 运行 --功能特性 http -- --transport http
```
...
1. **Use rmcp-macros**: Leverage `#[tool]``#[tool_router]```, an`, and `#[`#[tool_handler]`for cleaner code
2. **Type Safety**: Use `schemars::JsonSchema` for all parameter types
3. **错误 Handling**: Return `Result` types with proper 错误 messages
4. **Async/Await**: All handlers 必须 be async
...
7. **Logging**: Use `tracing` macros`info!``_CODE`warn!`rn!`_CO`错误!`_ `错误!`)
8. **Documentation**: 添加 doc comments 迁移到 all public items
...
Now 生成 the 完成 project based on the user's 环境要求!