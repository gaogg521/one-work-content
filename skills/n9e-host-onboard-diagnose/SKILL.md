...
**不进本 skill**：
- 曾经能看到、最近失联 → `n9e-host-health-diagnose`
- ident 重名 / 改名后想清理残留 → `n9e-host-ident-cleanup`（待建）
- 想改告警规则 / 屏蔽 → `creation` / `n9e-create-alert-rule`
- 看告警没发出来为啥 → `n9e-alert-rule-troubleshoot`
...
只有 (2) 或 (3) 有数据 → **ident 标签问题**（特殊字符 / global_labels 覆盖 / snmp agent_host_tag 误用），引导用户走 `n9e-host-ident-cleanup`（待建）或修 categraf 配置。
