---
name: vibetunnel
description: 管理 VibeTunnel 终端会话。创建、列出、监控和控制 VibeTunnel Web 仪表板中可见的终端会话。
...
通过 REST API 管理 [VibeTunnel](https://github.com/AugmentedMomentum/vibetunnel) 终端会话。创建、列出、监控和控制 Web 仪表板中可见的会话。

## 设置

VibeTunnel 必须正在运行。默认：`http://localhost:8080`。使用 `VT_URL` 环境变量覆盖。
...
参数：
- `command`: array —— 命令 + 参数 (默认: `["zsh", "-l", "-i"]`)
- `name`: string —— 显示名称
- `workingDir`: string —— 工作目录
- `cols`: number —— 终端宽度 (默认: 120)
- `rows`: number —— 终端高度 (默认: 30)
