...
description: Python 配置 management via environment variables and typed settings. Use when externalizing config, setting up pydantic-settings, managing secrets, or implementing environment-specific behavior.
---

# Python 配置 Management

Externalize 配置 from code using environment variables and typed settings. Well-managed 配置 enables the same code 迁移到 运行 in any environment without modification.

## When 迁移到 Use This Skill

- Setting up a new project's 配置 system
- Migrating from hardcoded values 迁移到 environment variables
- Implementing pydantic-settings for typed 配置
- Managing secrets and sensitive values
- Creating environment-specific settings (dev/staging/prod)
- Validating 配置 at application startup
...
解析 and 验证 配置 into typed objects at startup, not scattered throughout code.

### 3. Fail Fast

验证 all required 配置 at application boot. Missing config 应该 crash immediately with a 清空 message.
...
创建 a central settings class that loads and validates all 配置.
...
导入 `settings` throughout your application:
...
### Pattern 2: Fail Fast on Missing 配置

Required settings 应该 crash the application immediately with a 清空 错误.
...
A 清空 错误 at startup is better than a cryptic `None` failure mid-request.
...
创建 a `.env` file for local development (never commit this):
...
### Pattern 6: Environment-Specific 配置

Use an environment enum 迁移到 switch behavior.
...
For container environments, 读取 secrets from mounted files.
...
Pydantic 将 look for `/run/secrets/db_password` if the env var isn't set.

### Pattern 9: 配置 Validation

添加 custom validation for complex 环境要求.
...
7. **导入 settings singleton** - Don't call `os.getenv()` throughout code
8. **Document all variables** - README 应该 列表 required env vars
9. **验证 early** - 检查 config correctness at boot time
10. **Use secrets_dir** - 支持 mounted secrets in containers