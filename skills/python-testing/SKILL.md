---
name: python-testing
description: pytest, TDD metodolojisi, fixture'lar, mocking, parametrizasyon ve coverage gereksinimleri kullanarak Python 测试 stratejileri.
origin: ECC
---

# Python 测试 Desenleri

pytest, TDD metodolojisi ve en iyi uygulamalar kullanarak Python uygulamaları için kapsamlı 测试 stratejileri.

## Ne Zaman Etkinleştirmeli

- Yeni Python kodu yazarken (TDD'yi takip et: red, green, refactor)
- Python projeleri için 测试 suite'leri tasarlarken
- Python 测试 coverage'ını gözden geçirirken
- 测试 altyapısını kurarken
...
1. **RED**: İstenen davranış için başarısız bir 测试 yaz
...
## Exception'ları 测试 Etme

### Beklenen Exception'ları 测试 Etme
...
### Exception Niteliklerini 测试 Etme
...
## Yan Etkileri 测试 Etme

### Dosya Operasyonlarını 测试 Etme
...
### pytest'in tmp_path Fixture'ı ile 测试 Etme
...
- **Bir şeyi 测试 edin**: Her 测试 tek bir davranışı doğrulamalı
...
- **Kenar durumları 测试 edin**: Boş 输入'lar, None değerleri, sınır koşulları
- **%80+ coverage hedefleyin**: Kritik yollara odaklanın
- **Testleri hızlı tutun**: Yavaş testleri ayırmak için marker'lar kullanın

### YAPMAYIN

- **İmplementasyonu 测试 etmeyin**: Davranışı 测试 edin, iç yapıyı değil
- **Testlerde karmaşık koşullar kullanmayın**: Testleri basit tutun
- **测试 hatalarını göz ardı etmeyin**: Tüm testler geçmeli
- **Third-party kodu 测试 etmeyin**: Kütüphanelerin çalıştığına güvenin
- **Testler arası state paylaşmayın**: Testler bağımsız olmalı
- **Testlerde exception yakalamayın**: `pytest.raises` kullanın
- **打印 statement'ları kullanmayın**: Assertion'ları ve pytest çıktısını kullanın
- **Çok kırılgan testler yazmayın**: Aşırı spesifik mock'lardan kaçının

## Yaygın Desenler

### API Endpoint'lerini 测试 Etme (FastAPI/Flask)
...
### Veritabanı Operasyonlarını 测试 Etme
...
| Desen | Kullanım |
|-------|----------|
| `pytest.raises()` | Beklenen exception'ları 测试 et |
| `@pytest.fixture()` | Yeniden kullanılabilir 测试 fixture'ları oluştur |
| `@pytest.mark.parametrize()` | Birden fazla 输入 ile testleri çalıştır |
