---
name: k8s-kind
description: kind 本地集群管理专家 - 在 Docker 中创建、测试和开发 Kubernetes 本地集群
license: Apache-2.0
metadata:
  author: rohitg00
  version: 1.0.0
...
管理 local Kubernetes clusters using kubectl-mcp-server's kind tools (32 tools).
...
在以下情况下使用此技能:
- 用户 mentions: "kind", "local cluster", "Kubernetes in Docker", "dev cluster"
...
|----------|------|--------|-------|
| 1 | Detect kind CLI first | 严重 | `kind_detect_tool` |
| 2 | Generate config for complex setups | 高 | `kind_config_generate_tool` |
| 3 | Load images after cluster creation | 高 | `kind_load_image_tool` |
| 4 | Export logs for debugging | 中 | `kind_export_logs_tool` |
...
- **kind CLI**: 必需 for all kind tools
  ```bash
  # macOS
  brew install kind
  # or download binary
  ...
  ```
- **Docker**: 必需 (kind runs clusters in Docker containers)
...
## kind vs vcluster (vCluster)

| Feature | kind | vcluster (vCluster) |
|---------|------|-----------------|
| 用途 | Local dev/CI clusters | Virtual clusters in existing K8s |
...
- [k8s-vcluster](../k8s-vcluster/SKILL.md) - vCluster management
