...
description: 搜索并获取 YouTuber 信息，包括订阅数、增长统计、定价估算等。
...
搜索并获取 YouTuber 信息，包括订阅数、增长统计、定价估算等。
...
- 需要环境变量 `CREATORDB_API_KEY`
- 或在 `~/.clawdbot/moltbot.json` 中设置 `skills."creatordb-youtube-v3".env.CREATORDB_API_KEY`
- 在 https://www.creatordb.app 获取你的 API key

## 备注

- 使用搜索结果中的 `channelId` 获取详细资料
- `subscriberGrowth`：`g7`/`g30`/`g90` = 7/30/90 天的增长率
- `videoPrice`/`shortsPrice`：以 USD 估算的赞助定价
- `categoryBreakdown`：频道内容类别分布
- 分页：搜索结果使用 `offset` 和 `pageSize`
